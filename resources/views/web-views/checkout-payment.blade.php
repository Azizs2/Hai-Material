...
@push('css_or_js')
    ...
    {{-- popup checkout --}}
    <link rel="stylesheet" href="{{asset('public/assets/checkout')}}/checkout.css"/>
    {{-- popup checkout --}}
...
@section('content')
    ...
    <div class="container pb-5 mb-2 mb-md-4 rtl" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
        <div class="row">
            ...
            <section class="col-lg-8">
                ...
                <!-- Payment methods accordion-->
                    ...
                    <div class="row">
                                    ...
                                    <div class="card-body" style="height: 100px">
                                        <form action="{{route('checkout-complete',['payment_method'=>'cash_on_delivery'])}}" method="get" class="needs-validation">
                                            <button class="btn btn-block click-if-alone" type="submit">
                                                <img width="150" style="margin-top: -10px" src="{{asset('public/assets/front-end/img/cod_new.png')}}"/>
                                            </button>
                                        </form>
                                    </div>
                                </div>
                        ...
                        {{-- transfer --}}
                        {{-- @php($config=\App\CPU\Helpers::get_business_settings('transfer')) --}}
                        {{-- @if($config['status']) --}}
                            <div class="col-md-6 mb-4" style="cursor: pointer">
                                <div class="card">
                                    <div class="card-body" style="height: 100px">
                                        {{-- <form action="{{route('checkout-transfer',['payment_method'=>'transfer'])}}" method="get" class="needs-validation"> --}}
                                            <button class="btn btn-block click-if-alone"  data-target="#bank_transfer" data-toggle="modal" type="button">
                                                <img width="150" style="margin-top: -10px" src="{{asset('public/assets/front-end/img/transfer.png')}}"/>
                                            </button>
                                        {{-- </form> --}}
                                    </div>
                                </div>
                                        ...
                                        <form method="POST" action="{{ route('paystack-pay') }}" accept-charset="UTF-8" class="form-horizontal" role="form">
                                            ...
                                            <div class="row">
                                                <div class="col-md-8 col-md-offset-2">
                                                    <input type="hidden" name="email" value="{{auth('customer')->user()->email}}"> {{-- required --}}
                                                    <input type="hidden" name="orderID" value="{{session('cart_group_id')}}">
                                                    <input type="hidden" name="amount" value="{{\App\CPU\Convert::usdTozar($amount*100)}}"> {{-- required in kobo --}}
                                ...
                                <div class="card">
                                    <div class="card-body" style="height: 100px">
                                        <form class="needs-validation" method="POST" id="payment-form-paymob" action="{{route('paymob-credit')}}">
                                            {{ csrf_field() }}
                                            <button class="btn btn-block click-if-alone" type="submit">
                                                <img width="150" src="{{asset('public/assets/front-end/img/paymob.png')}}"/>
                                            </button>
                                        </form>
                                    </div>
                                </div>
                                ...
                                <div class="card">
                                    <div class="card-body" style="height: 100px">
                                        <a class="btn btn-block click-if-alone" href="{{route('paytm-payment')}}">
                                            <img style="max-width: 150px; margin-top: -10px" src="{{asset('public/assets/front-end/img/paytm.png')}}"/>
                                        </a>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card">
                                    <div class="card-body" style="height: 100px">
                                        <a class="btn btn-block click-if-alone" href="{{route('liqpay-payment')}}">
                                            <img style="max-width: 150px; margin-top: 0px" src="{{asset('public/assets/front-end/img/liqpay4.png')}}"/>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        ...
                        {{-- bank_mandiri --}}
                        {{-- @php($config=\App\CPU\Helpers::get_business_settings('bank_mandiri'))
                        @if(isset($config) && $config['status'])
                            <div class="col-md-6 mb-4" style="cursor: pointer">
                                <div class="card">
                                    <div class="card-body" style="height: 100px">
                                        <a class="btn btn-block click-if-alone" href="{{route('mandiri')}}">
                                            <img width="150" src="{{asset('public/assets/front-end/img/bank_mandiri.png')}}"/>
                                        </a>
                                        <button class="btn btn-block click-if-alone" data-target="#bank_mandiri" data-toggle="modal">
                                            <img style="max-width: 150px; margin-top: 0px" src="{{asset('public/assets/front-end/img/transfer.png')}}"/>
                                        </button>
                                    </div>
                                </div>
                            </div>
            ...
            {{-- bank transfer --}}
            <div class="modal fade rtl" style="text-align: left;" id="bank_transfer" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content" style="border-radius: 2em;">
                        <div class="modal-header">
                            <h5 class="modal-title font-nameA "  id="exampleModalLabel">{{\App\CPU\translate('checkout')}}</h5>
                        ...
                        <div class="modal-body">
                            <div class="form-row">
                                <div class="form-group" style="width: 100%">
                                    <label for="firstName">{{\App\CPU\translate('total')}}</label>
                                    <div class="card" style="background-color:#6a1d35;">
                                        <h1 style="padding-top: 10px; text-align: center; color:white;">{{$mandiris['currency']}}. {{$mandiris['amount']}}</h1>
                                    </div>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group" style="width: 100%">
                                    <label for="firstName">{{\App\CPU\translate('Transfer Bank<')}}</label>
                                    <div class="card" style="height: 60px;">
                                        <div style="display: flex; align-items: center;">
                                            <img width="80" src="{{asset('public/assets/front-end/img/bank_mandiri.png')}}"/>
                                            <label style="padding-top: 10px; padding-left:5px">PT. Sentra Media Investama</label>
                                        </div>
                                        <h6 style="text-align: center; font-weight: bold;">12334567</h6>
                                    </div>
                                    <div class="card" style="height: 60px;margin-top:5px">
                                        <div style="display: flex; align-items: center;">
                                            <img width="80" src="{{asset('public/assets/front-end/img/bank_bca.png')}}"/>
                                            <label style="padding-top: 10px; padding-left:5px">PT. Sentra Media Investama</label>
                                        </div>
                                        <h6 style="text-align: center; font-weight: bold;">12334567</h6>
                                    </div>
                                    <div class="card" style="height: 60px;margin-top:5px">
                                        <div style="display: flex; align-items: center;">
                                            <img width="80" src="{{asset('public/assets/front-end/img/bank_bni.png')}}"/>
                                            <label style="padding-top: 10px; padding-left:5px">PT. Sentra Media Investama</label>
                                        </div>
                                        <h6 style="text-align: center; font-weight: bold;">12334567</h6>
                                    </div>
                                    <div class="card" style="height: 60px;margin-top:5px">
                                        <div style="display: flex; align-items: center;">
                                            <img width="80" src="{{asset('public/assets/front-end/img/bank_btn.png')}}"/>
                                            <label style="padding-top: 10px; padding-left:5px">PT. Sentra Media Investama</label>
                                        </div>
                                        <h6 style="text-align: center; font-weight: bold;">12334567</h6>
                                    </div>
                            ...
                            <div class="modal-footer" style="padding: 0px!important;">
                                ...
                                @if($config['status'])
                                    <form action="{{route('checkout-transfer',['payment_method'=>'transfer'])}}" method="get" class="needs-validation">
                                        <button type="submit" class="btn btn-primary">{{\App\CPU\translate('checkout')}}</button>
                                    </form>
                                @endif
                            </div>
                        </div>
